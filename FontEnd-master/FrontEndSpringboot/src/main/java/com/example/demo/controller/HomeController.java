...
public class HomeController extends HttpServlet {
		...
		for (int i = 1; en.hasMoreElements(); i++) {
			...
			if (param.contains("form18")) {
				...
				if (indexForm18 == 3) {
					...
					jsonForm18.put("image1", getImagePath(passPort, "image1"));
				...
				if (indexForm18 == 7) {
					...
					jsonForm18.put("birthday", values[i]);
					jsonForm18.put("image2", getImagePath(passPort, "image2"));
	...
	private static String saveImageToFolder(String namePassPort) throws IOException {
		...
		String imagePath = getImagePath(namePassPort, filterImage);

		return imagePath;
	}

	// get image path
	private static String getImagePath(String namePassPort, String filterImage) {

		String pathPassPortFolder = Configure.PATH + namePassPort;

		File filePath = new File(pathPassPortFolder);
		
		System.out.println("File Path: " + pathPassPortFolder);
			...
			for (File file : allSubFiles) {
				...
				if (file.isFile()) {
					...
					if (f.contains(filterImage)) {
						System.out.println("f: " + namePassPort + f.substring(f.lastIndexOf("\\"), f.length()));
						return namePassPort + f.substring(f.lastIndexOf("\\"), f.length());
					}
