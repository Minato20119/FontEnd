...
public class GetDataFormHTML extends HttpServlet {
	...
	public static final String PATH = "C:\\Users\\Minato\\eclipse-workspace\\FontEnd2\\WebContent\\WEB-INF\\upload\\";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String querry = "SELECT form12_passport FROM mydb.dataform";
	...
	private static void insertDatabase(Statement st, String imagePath) throws SQLException, ParseException {
		...
		if (querry.contains(",")) {
			...
			querry = querry.substring(0, querry.length()) + " image, " + "start_time) ";

			if (value.contains(",")) {
				value = value.substring(0, value.length()) + "\"" + imagePath + "\"" + ", \"" + strDate + "\");";
			}
	...
	public static String saveImageToFolder(String passPortFolder) throws IOException {

		new File(PATH + passPortFolder).mkdir();

		System.out.println("Done create directory!");

		File sourceFile = new File(PATH + "image.jpg");
		
		sourceFile.renameTo(new File(PATH + passPortFolder + "\\image.jpg"));
		
		System.out.println("Done move file!");

		return new File(PATH + passPortFolder + "\\image.jpg").getAbsolutePath();
	}

	public static void removeImageToFolderUpload() {
		try {
