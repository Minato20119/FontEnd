...
public class GetDataFormHTML extends HttpServlet {
	...
	public static final String PATH = "C:\\Users\\Minato\\eclipse-workspace\\FontEnd3\\WebContent\\WEB-INF\\upload\\";
	...
	private static void insertDatabase(Statement st, String imageJson) throws SQLException, ParseException {
		...
		if (querry.contains(",")) {
			...
			if (value.contains(",")) {
				value = value.substring(0, value.length()) + "'" + imageJson + "'" + ", \"" + strDate + "\");";
			}
	...
	private static String saveImageToFolder(String passPortFolder) throws IOException, JSONException {
		...
		for (File file : allSubFiles) {
			if (file.isFile()) {
				...
				sourceFile.renameTo(new File(PATH + passPortFolder + "\\" + f.substring(f.lastIndexOf("\\"), f.length())));
			}
		}
